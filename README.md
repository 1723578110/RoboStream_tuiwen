...
图 1：SoFar、VoxPoser 与 RoboStream 在长程任务中的对比。前三列展示 block building、block disassembly 和 block hide-and-restore 中的真实机器人执行，后三列展示对应的模拟任务。失败通常来自跨步骤状态丢失，而不是最后一步才突然出错。
...
图 2：RoboStream 整体框架。系统从 RGB-D 观测中构建 STF-Tokens，再更新 CSTG，并将结构化记忆送入 VLM 进行长程规划。
...
图 3：真实机器人实验设置。系统在 FR3 机械臂和 RealSense RGB-D 相机上进行长程积木操作。
...
图 4：Hard 级真实长程积木操作。任务要求机器人在多物体、多步骤场景中维持结构稳定和正确操作顺序。
...
图 5：Hide-and-Restore 真实任务。目标被完全遮挡后，RoboStream 仍能依靠因果记忆恢复隐藏对象并还原场景。
